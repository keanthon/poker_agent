...
} from '/var/task/Documents/keanthon/poker_agent/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Documents/keanthon/poker_agent/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Documents/keanthon/poker_agent')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Documents/keanthon/poker_agent',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Documents/keanthon/poker_agent/.netlify/dist/run/handlers/server.js')
